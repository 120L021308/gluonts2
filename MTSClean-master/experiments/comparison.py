...
import sys
sys.path.append(r'E:\72fd7dbf\shiyan\MTSClean-master')
print(sys.path)
...
def plot_segment(dm, col, start, end, buffer, cleaned_results, row_constraints):
    ...
    for _, coefs, rho_min, rho_max in row_constraints:
        if coefs[col_index] != 0:  # 判断当前行约束是否与col有关
            lower_bound, upper_bound = calculate_correct_value_range(dm, coefs, rho_min, rho_max, col_index, plot_start,
                                                                     plot_end)
            ax.fill_between(range(plot_start, plot_end), lower_bound, upper_bound, color='yellow', alpha=0.3,
...
def evaluate_cleaning_algorithms_by_segment_length(data_manager):
    ...
    for ratio in segment_ratios:
        ...
        for name, algo in algorithms.items():
            ...
            cleaned_data = algo.clean(dm_copy, row_constraints=row_constraints, speed_constraints=speed_constraints,
                                      accel_constraints=accel_constraints)
    ...
    save_results_to_csv(violation_ratio_by_algorithm, segment_ratios,
                        f'{data_manager.dataset}_violation_ratio_by_segment_length.csv')
...
def evaluate_cleaning_algorithms_by_error_ratio(data_manager):
    ...
    save_results_to_csv(violation_ratio_by_algorithm, error_ratios,
                        f'{data_manager.dataset}_violation_ratio_by_error_ratio.csv')
