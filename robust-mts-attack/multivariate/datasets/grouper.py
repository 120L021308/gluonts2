...
class Grouper:
    ...
    def _prepare_test_data(self, dataset):
        ...
        print([entry.shape for entry in grouped_entry])  # 打印所有子项的形状

        # 这里是我加的，假设 grouped_entry 是包含这些不规则数据行的列表
        max_valid_cols = 4000  # 你想保留的有效列数

        # 遍历每一行，移除前面的 0 填充，保持 4000 列
        grouped_entry = np.array([entry[-max_valid_cols:] for entry in grouped_entry])
