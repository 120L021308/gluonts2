...
def download_dataset(dataset_path: Path, ds_info):
    # 确保目录存在
    os.makedirs(dataset_path, exist_ok=True)

    # 完整的文件路径
    file_path = dataset_path / f"{ds_info.name}.tar.gz"

    # 打印文件路径检查是否正确
    print("下载文件将保存到：", file_path)

    # 执行下载
    request.urlretrieve(ds_info.url, str(file_path))


# 示例调用
dataset_path = Path(r"E:\datasets\traffic_nips")  # 使用原始字符串避免路径问题
ds_info = GPCopulaDataset(
    name="traffic_nips",
    url="https://path/to/your/dataset.tar.gz",
    num_series=963,
    prediction_length=24,
    freq="H",
    rolling_evaluations=7
)
download_dataset(dataset_path, ds_info)
